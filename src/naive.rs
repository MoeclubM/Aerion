...
fn naive_tls_config(
    ...
    let mut tls_config =
        Arc::unwrap_or_clone(tls::client_config_with_custom_root_material_options(
            config.insecure,
...
fn build_naive_quic_endpoint(
    ...
    let mut tls = Arc::unwrap_or_clone(tls::client_config_with_custom_root_material_options(
        config.insecure,
        &config.ca_cert_paths,
        &config.ca_certificates,
        config.disable_system_roots,
